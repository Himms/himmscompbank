...
def update(user, amount):
    from string import whitespace
    import re
    print("update user record")
    # find user with account number
    f = open(user_db_path + str(user) + ".txt", "r+")
    user_list = str.split(f.readline(),',')
    user_list[4]= amount
    user_list = str(user_list)
    user_list = user_list.replace("'","")
    user_list = user_list.replace(" ","")
    user_list = user_list.strip(whitespace + "[]")
    print(user_list)
    user_list= re.sub(user_list, user_list, user_list)
    f.seek(0)
    return f.write(user_list)
